  ...
  return (
    <div className="relative mb-8 bg-gradient-to-r from-wang-blue/10 to-wang-blue/5 rounded-xl p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 z-10 relative">
        <div>
          <h1 className="text-2xl md:text-3xl font-prompt font-bold text-wang-blue">{title[language]}</h1>
          <p className="text-gray-600 mt-2 max-w-xl font-sarabun">{description[language]}</p>
          <div className="flex flex-wrap gap-2 mt-3">
            <Badge variant="outline" className="bg-white/80"><Building2 className="mr-1 h-3 w-3" />{totalCompanies} {language === 'th' ? 'บริษัท' : 'Companies'}</Badge>
            <Badge variant="outline" className="bg-white/80">{totalJobs} {language === 'th' ? 'ตำแหน่งงาน' : 'Open Positions'}</Badge>
          </div>
        </div>
