...
const CompanyTabSection: React.FC<CompanyTabSectionProps> = ({ companies, filteredCompanies, verifiedCompanies }) => {
  ...
  const activeCompanyList = useMemo(() => {
    if (activeTab === 'filtered') return filteredCompanies;
    if (activeTab === 'verified') return verifiedCompanies;
    return companies;
  }, [activeTab, companies, filteredCompanies, verifiedCompanies]);

  const { currentPage, totalPages, goToPage, goToNext, goToPrevious, pageNumbers, startIndex, endIndex, reset } = usePagination({ totalItems: activeCompanyList.length, itemsPerPage: 6 });
    ...
    return (
      <>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {companyList.map((company) => <CompanyCard key={company.id} company={company} />)}
        </div>
        {totalPages > 1 && (
            ...
            <PaginationContent>
              ...
              {pageNumbers.map((page, index) => (
                <PaginationItem key={index}>
                  {typeof page === 'string' ? <span className="px-4 py-2">...</span> : <PaginationLink isActive={page === currentPage} onClick={() => goToPage(page)} className="cursor-pointer">{page}</PaginationLink>}
                </PaginationItem>
              ))}
